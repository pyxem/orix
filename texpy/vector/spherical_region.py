...
class SphericalRegion:
    ...
    def from_symmetry(cls, symmetry):
        ...
        if mirror_planes_z.size:
            order = np.lexsort([mirror_planes_z.y, mirror_planes_z.dot(-Vector3d.xvector())])
        ...
        if mirror_planes_z.size:
            angles = mirror_planes_z.angle_with(mirror_plane_z_1)
            angles[np.isclose(angles, 0)] = np.pi
            signs = np.sign(mirror_planes_z.cross(mirror_plane_z_1).z)
            ...
            if mirror_plane_z_1.size:
                if mirror_plane_z_2.size:
                    mplanenz = mirror_planes_nz[
                        mirror_planes_nz.dot(mirror_plane_z_1 + mirror_plane_z_2).argmin()]
                else:
                    mplanenz = mirror_planes_nz[mirror_planes_nz.dot(mirror_plane_z_1).argmin()]
            else:
        ...
        while r_inside.size:
            size = r_inside.size
            r = r_inside[np.isclose(r_inside.angle, r_inside.angle.min())]
            r = r[r.axis.angle_with(Vector3d.zvector()).argmin()]
            if np.isclose(r.axis.z, 1):
                perp = Vector3d.yvector()
            elif np.isclose(r.axis.x, 1):
                perp = Vector3d.zvector()
            elif np.isclose(r.axis.y, 1):
                perp = Vector3d.xvector()
            else:
                if np.isclose(np.sum(supersector.normals.z), 0):
                    perp = Vector3d.yvector()
                else:
                    perp = Vector3d.zvector()
    ...
    def contains(self, vectors, edge=True):
        ...
        return SphericalRegion.is_in_or_on(self.normals, vectors, comparator)
