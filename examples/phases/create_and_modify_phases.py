...
This example shows various ways to both create and modify :class:`~orix.crystal_map.Phase`
and :class:`~orix.crystal_map.PhaseList` objects.
...
# %%
# As seen in the ferrite and autenite phases above, Phases can be defined from
...
# and will automatically add the correct point group to the Phase as well.
phase_i3m = ocm.Phase(space_group=165)
print(phase_i3m)

# %%
# Alternatel, a Phase can be defined using a point group, in which case the
...
# from a standalone and/or new Phase, as well as deleted. There is also a
...
# %% By default, the colors for both a single Phase or a Phaselist will be
# populated from the matplotlib defaults. these can be overwritten using valid
# matplotlib color strings, which can be found on matplotlib's website:
