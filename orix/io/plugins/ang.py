...
def file_reader(filename: str, autogen_names: Optional[bool] = True) -> CrystalMap:
    ...
    * EDAX TSL: confidence index == -1
    ...
    # If requested, replace PhaseList.names with autogenerated values.
    if autogen_names:
        _autogen_phase_names(data_dict["phase_list"], formulas, phases["ids"])
...
def _autogen_phase_names(phase_list, formulas, ids):
    # renaming requires a formula for each phase as a minimum.
    if len(formulas) != phase_list.size:
        return phase_list
    # Resort the formulas to match the order in the PhaseList
    names = np.array(formulas)[np.argsort(ids)]
    # if the formula are not unique, include the point group in the name
    if np.unique(names, return_counts=True)[1].max() > 1:
        names = [
            names[i] + "-" + phase_list[j].point_group.name
            for i, j in enumerate(phase_list.ids)
        ]
    # if this still doesn't create a unique name, add a letter to the end.
    if np.unique(names, return_counts=True)[1].max() > 1:
        names = [names[i] + "-" + chr(65 + i) for i in range(len(names))]
