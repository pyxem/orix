...
class TestGetSampleLocal:
    ...
    def test_get_sample_local_center(self, fixed_rotation, width):
        """Checks that the center argument works as expected."""
        res = 8
        x = get_sample_local(resolution=res, center=fixed_rotation, grid_width=width)
        angles = x.angle_with(fixed_rotation).data
        assert np.all(angles < np.deg2rad(width))
        # Makes sure some of our rotations are inner the outer region
        assert np.any(angles > np.deg2rad(width - res * 1.5))
