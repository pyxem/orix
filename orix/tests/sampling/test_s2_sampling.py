...
class TestS2Sampling:
    ...
    def test_uv_mesh(self, resolution, size):
        v1 = S2_sampling._sample_S2_uv_mesh(resolution)
        assert v1.size == size
    ...
    def test_cube_mesh(self, grid_type, resolution, size):
        v1 = S2_sampling._sample_S2_cube_mesh(resolution, grid_type=grid_type)
        assert v1.size == size
        assert np.allclose(v1.mean().data, [0, 0, 0])
    ...
    def test_cube_mesh_raises(self):
        with pytest.raises(ValueError, match="The `grid_type` hexagonal"):
            _ = S2_sampling._sample_S2_cube_mesh(2, "hexagonal")

    def test_random_sphere_mesh(self):
        grid = S2_sampling._sample_S2_random_mesh(1).data
        assert grid.shape[0] == 10313
        assert grid.shape[1] == 3

    def test_seed_for_random_sphere_mesh(self):
        grid_7 = S2_sampling._sample_S2_random_mesh(resolution=3, seed=7)
        grid_7_again = S2_sampling._sample_S2_random_mesh(resolution=3, seed=7)
        grid_8 = S2_sampling._sample_S2_random_mesh(resolution=3, seed=8)
    ...
    def test_hexagonal_mesh(self):
        grid = S2_sampling._sample_S2_hexagonal_mesh(5).data
        assert grid.shape == (3458, 3)

    def test_icosahedral_grid(self):
        grid = S2_sampling._sample_S2_icosahedral_mesh(10).data
    ...
    def test_sample_s2(self):
        ...
        grid_2 = S2_sampling._sample_S2_hexagonal_mesh(resolution)
